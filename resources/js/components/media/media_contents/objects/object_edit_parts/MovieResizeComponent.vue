...
<script>
  ...
  export default {
    ...
    watch : {},
    methods : {
      // ...mapMutations('mediaMovie', ['setTargetObjectIndex']),
      ...mapMutations('mediaMovie', ['updateMediaMovieObjectItem']),

      // getOneMovie(index){ // ストアから自分のインデックスのオブジェクトだけ取得する
      //   this.setTargetObjectIndex(index);
      //   return this.getMediaMovie;
      // },
      resizeWrapperSize(){
      ...
      resizeRight(e){
        const left = this.getMediaMovie['left'];
        this.width = e.clientX - left;
        this.updateMediaMovieObjectItem({index:this.index,key:"width",value:this.width});
        this.resizeMiddleLast();
      },
      resizeLeft(e){
        const x = this.getMediaMovie['left'];
        const diff = x - e.clientX;
        const width_before = this.getMediaMovie['width'];
        const width_new = width_before + diff;
        this.updateMediaMovieObjectItem({index:this.index,key:"width",value:width_new});
        this.width = width_new;
        const new_x = x - diff;
        this.updateMediaMovieObjectItem({index:this.index,key:"left",value:new_x});
        this.resizeMiddleLast();
      },
      resizeBottom(e){
        const top = this.getMediaMovie['top'];
        this.height = e.clientY - top;
        this.updateMediaMovieObjectItem({index:this.index,key:"height",value:this.height});
        this.resizeMiddleLast();
      },
      resizeTop(e){
        const y = this.getMediaMovie['top'];
        const diff = y - e.clientY;
        const height_before = this.getMediaMovie['height'];
        const height_new = height_before + diff;
        this.updateMediaMovieObjectItem({index:this.index,key:"height",value:height_new});
        this.height = height_new;
        const new_y = y - diff;
        this.updateMediaMovieObjectItem({index:this.index,key:"top",value:new_y});
        this.resizeMiddleLast();
      },
